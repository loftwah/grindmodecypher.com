        ...
        function ac_load_additional_controls_js() {
            ...
            if ( version_compare( $wp_version, '5.4.2' , '>=' ) ) {
                wp_localize_script(
                  ...
                  array(
                    'clear'            => __( 'Clear', 'hueman-pro' ),
                    'clearAriaLabel'   => __( 'Clear color', 'hueman-pro' ),
                    'defaultString'    => __( 'Default', 'hueman-pro' ),
                    'defaultAriaLabel' => __( 'Select default color', 'hueman-pro' ),
                    'pick'             => __( 'Select Color', 'hueman-pro' ),
                    'defaultLabel'     => __( 'Color value', 'hueman-pro' )
                  )
                );
            }
                ...
                apply_filters( 'czr_js_customizer_control_params' ,
                  array(
                      ...
                      'i18n' => array(
                            ...
                            'select_image'        => __( 'Select Image', 'hueman-pro' ),
                            'change_image'        => __( 'Change Image', 'hueman-pro' ),
                            'remove_image'        => __( 'Remove', 'hueman-pro' ),
                            'default_image'       => __( 'Default', 'hueman-pro'  ),
                            'placeholder_image'   => __( 'No image selected', 'hueman-pro' ),
                            'frame_title_image'   => __( 'Select Image', 'hueman-pro' ),
                            'frame_button_image'  => __( 'Choose Image', 'hueman-pro' ),

                            'Customizing' => __('Customizing', 'hueman-pro'),
                      ),
        ...
        function ac_set_ajax_czr_tmpl() {
            ...
            if ( !check_ajax_referer( $action, 'nonce', false ) ) {
                 wp_send_json_error( array(
                  'code' => 'invalid_nonce',
                  'message' => __( 'ac_set_ajax_czr_tmpl => Security check failed.', 'hueman-pro' ),
                ) );
            }
        ...
        function ac_get_all_modules_tmpl( $html, $requested_tmpl = '', $posted_params = array() ) {
            ...
            switch ( $requested_tmpl ) {
                case 'crud-module-part' :
                    ?>
                      <button class="<?php echo $css_attr['open_pre_add_btn']; ?>"><?php _e('Add New', 'hueman-pro'); ?> <span class="fas fa-plus-square"></span></button>
                      <div class="<?php echo $css_attr['pre_add_wrapper']; ?>">
                        ...
                        <div class="<?php echo $css_attr['pre_add_item_content']; ?>">
                          ...
                          <span class="<?php echo $css_attr['cancel_pre_add_btn']; ?> button"><?php _e('Cancel', 'hueman-pro'); ?></span> <span class="<?php echo $css_attr['add_new_btn']; ?> button"><?php _e('Add it', 'hueman-pro'); ?></span>
                        </div>
                      </div>
                    <?php
                break;
                case 'rud-item-part' :
                    ?>
                      <div class="<?php echo $css_attr['item_header']; ?> czr-custom-model">
                        ...
                        <# } #>
                        <div class="<?php echo $css_attr['item_btns']; ?>"><a title="<?php _e('Edit', 'hueman-pro'); ?>" href="javascript:void(0);" class="fas fa-pencil-alt <?php echo $css_attr['edit_view_btn']; ?>"></a>&nbsp;<a title="<?php _e('Remove', 'hueman-pro'); ?>" href="javascript:void(0);" class="fas fa-trash <?php echo $css_attr['display_alert_btn']; ?>"></a></div>
                ...
                case 'rud-item-alert-part' :
                    ?>
                      <p class="czr-item-removal-title"><?php _e('Are you sure you want to remove : <strong>{{ data.title }} ?</strong>', 'hueman-pro'); ?></p>
                      <span class="<?php echo $css_attr['remove_view_btn']; ?> button"><?php _e('Yes', 'hueman-pro'); ?></span> <span class="<?php echo $css_attr['cancel_alert_btn']; ?> button"><?php _e('No', 'hueman-pro'); ?></span>
                    <?php
                break;
    ...
    class CZR_Fmk_Base extends CZR_Fmk_Dyn_Module_Registration {
      ...
      public function ajax_load_available_items() {
            ...
            if ( !check_ajax_referer( $action, 'nonce', false ) ) {
                 wp_send_json_error( array(
                  'code' => 'invalid_nonce',
                  'message' => __( 'ajax_load_available_items => Security check failed.', 'hueman-pro' ),
                ) );
            }
            ...
            if ( 'post' === $type ) {
                  ...
                  foreach ( $posts as $post ) {
                        ...
                        if ( '' === $post_title ) {
                          ...
                          $post_title = sprintf( __( '#%d (no title)', 'hueman-pro' ), $post->ID );
                        }
                        $items[] = array(
      ...
      public function ajax_search_available_items() {
            ...
            if ( empty( $items ) ) {
                wp_send_json_success( array( 'message' => __( 'No results found.', 'hueman-pro') ) );
            } else {
                wp_send_json_success( array(
