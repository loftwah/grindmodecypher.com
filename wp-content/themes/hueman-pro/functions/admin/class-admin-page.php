    ...
    function hu_print_upgrade_admin_notice() {
      ...
      <div class="notice notice-warning" style="display:block!important">
        <br/><br/>
            <?php
              printf(
                __( '<h2>As per the WordPress themes guidelines (%1$s), all the Hueman theme options have been moved to the %2$s.</h2>', 'hueman-pro' ),
                sprintf('<a href="%1$s" target="_blank">%2$s</a>', esc_url('https://make.wordpress.org/themes/handbook/review/required/#options-and-settings'), __('more information here', 'hueman-pro') ),
                sprintf('<a href="%1$s">%2$s</a>', admin_url('customize.php'), __('customizer', 'hueman-pro') )
              );
            ?>
          <p><?php _e("All your previous options have been kept and are available in the customizer options panel.", "hueman-pro" ) ?>
          <br/>
          <br/>
      </div>
      <?php
      ...
      function hu_welcome_panel() {
        ...
        <div id="hueman-admin-panel" class="wrap about-wrap">
          <?php
              $title = sprintf( '<h1 class="need-help-title">%1$s</h1>',
                sprintf( __( "Thank you for using %s %s :)", 'hueman-pro' ),
                  $_theme_name,
                  HUEMAN_VER
                )
              );
          ...
          <?php if ( $is_help ) : ?>
            ...
            <div class="changelog">
              ...
              <?php
                printf( '<p>%1$s</p>',
                  sprintf( __( "The best way to start is to read the %s." , 'hueman-pro' ),
                    sprintf('<a href="%1$s" title="%2$s" target="_blank">%2$s</a>', esc_url('docs.presscustomizr.com/article/236-first-steps-with-the-hueman-wordpress-theme'), __("knowledge base" , 'hueman-pro') )
                  )
                );
                if ( ! HU_IS_PRO ) {
                    // ob_start();
              ...
              <?php ob_start(); ?>
                  <div class="feature-section col two-col">
                    <div class="col">
                       <br/>
                        <a class="button-secondary hueman-help" title="documentation" href="<?php echo esc_url('docs.presscustomizr.com/') ?>" target="_blank"><?php _e( 'Read the documentation','hueman-pro' ); ?></a>
                    </div>
                    <?php if ( ! HU_IS_PRO ) : ?>
                        <div class="last-feature col">
                            <a class="button-secondary hueman-help" title="help" href="<?php echo esc_url('wordpress.org/support/theme/hueman'); ?>" target="_blank">
                              <?php _e( 'Get help in the free support forum','hueman-pro' ); ?>
                            </a>
                        </div>
                    <?php endif; ?>
                  </div><!-- .two-col -->
            ...
            <div class="about-text tc-welcome">
              <?php
                printf( '<p>%1$s %2$s</p> <p>%3$s. <strong>%4$s</strong></p>',
                  sprintf( __( "Thank you for using the %s theme for your website.", 'hueman-pro' ), $_theme_name ),
                  sprintf( __( "%s %s has more features, is safer and more stable than ever to help you designing an awesome webdesign.", 'hueman-pro' ), $_theme_name, HUEMAN_VER ),
                  sprintf( __( "For more information about this new version of the theme, %s or check the changelog below", "hueman-pro" ),
                    sprintf('<a href="%1$s" target="_blank">%2$s</a>', HU_WEBSITE . "/category/hueman-releases/", __( "read the latest release notes" , "hueman-pro" ) )
                  ),
                  sprintf('<a href="#hueman-changelog" title="%1$s">%1$s</a>', __( 'Changelog' , 'hueman-pro' ) )
                );

                printf( '<p><strong>%1$s</strong></p>',
                  sprintf( __( "The best way to start with %s is to read the %s and visit the %s.", 'hueman-pro'),
                    $_theme_name,
                    sprintf( '<a href="%1$s" title="%2$s" target="_blank" style="font-style:italic">%2$s</a>', esc_url('docs.presscustomizr.com/article/236-first-steps-with-the-hueman-wordpress-theme'), __("getting started guide", 'hueman-pro') ),
                    sprintf( '<a href="%1$s" title="%2$s" target="_blank">%2$s</a>', esc_url('demo-hueman.presscustomizr.com'), __("demo website", 'hueman-pro') )
                  )
                );
              ?>
            </div>
          ...
          <?php if ( is_child_theme() ) : ?>
            ...
            <div class="tc-upgrade-notice">
              <p>
              <?php
                printf( __('You are using a child theme of Hueman %1$s : always check the %2$s after upgrading to see if a function or a template has been deprecated.' , 'hueman-pro'),
                  'v'.HUEMAN_VER,
          ...
          <?php if ( ! HU_IS_PRO ) : ?>
              <div class="changelog">
                  ...
                  <div class="feature-section col two-col">
                    ...
                    <div class="col">
                      <h3 style="font-size:1.3em;"><?php _e( 'Happy user of Hueman?','hueman-pro' ); ?></h3>
                    ...
                    <div class="last-feature col">
                      <h3 style="font-size:1.3em;"><?php _e( 'Follow us','hueman-pro' ); ?></h3>
              ...
              <div id="extend" class="changelog">
                <h3 style="text-align:left;font-size:1.3em;"><?php _e("Go Hueman Pro" ,'hueman-pro') ?></h3>
                ...
                <div class="feature-section two-col images-stagger-right">
                  ...
                  <div class="col" style="float:left">
                    <h4 style="text-align: left;max-width:inherit"><?php _e('Easily take your web design one step further' ,'hueman-pro') ?></h4></br>

                    <p style="text-align: left;"><?php _e("The Hueman Pro WordPress theme allows anyone to create a beautiful, professional and mobile friendly website in a few minutes. In the Pro version, you'll get all features included in the free version plus many conversion oriented ones, to help you attract and retain more visitors on your websites." , 'hueman-pro') ?>
                    </p>
                    <p style="text-align:left;">
                        <a class="button-primary review-hueman hu-go-pro-btn" title="<?php _e("Discover Hueman Pro",'hueman-pro') ?>" href="<?php echo esc_url( HU_WEBSITE . '/hueman-pro?ref=a&utm_source=usersite&utm_medium=link&utm_campaign=hueman-admin-page' ); ?>" target="_blank"><?php _e("Discover Hueman Pro",'hueman-pro') ?> &raquo;</a>
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div class="return-to-dashboard">
          <a href="<?php echo esc_url( self_admin_url() ); ?>"><?php
            is_blog_admin() ? _e( 'Go to Dashboard &rarr; Home','hueman-pro' ) : _e( 'Go to Dashboard','hueman-pro' ); ?></a>
        </div>
...
<textarea readonly="readonly" onclick="this.focus();this.select()" id="system-info-textarea" name="tc-sysinfo" title="<?php _e( 'To copy the system infos, click below then press Ctrl + C (PC) or Cmd + C (Mac).', 'hueman-pro' ); ?>" style="width: 800px;min-height: 800px;font-family: Menlo,Monaco,monospace;background: 0 0;white-space: pre;overflow: auto;display:block;">
