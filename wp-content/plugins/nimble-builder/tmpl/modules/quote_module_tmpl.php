...
if ( !function_exists( __NAMESPACE__ . '\sek_print_quote_content' ) ) {
    function sek_print_quote_content( $quote_content, $input_id, $module_model ) {
        ...
        if ( skp_is_customizing() ) {
            $to_print = sprintf('<div title="%3$s" data-sek-input-type="textarea" data-sek-input-id="%1$s">%2$s</div>', esc_attr($input_id), $quote_content, __( 'Click to edit', 'nimble-builder' ) );
        } else {
